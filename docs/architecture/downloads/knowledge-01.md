...
# MT8676 / MT8668 智能座舱架构知识库

## 2026-09-20 修订导读

本版把学习范围扩展为 **MT8676 基线、MT8668 平台差异、盟博 OS 项目补图、PVT 实践与站点学习方法**。先读 [主题导航](README.md)，再按问题进入章节。00—16 的原始资料主体继续保留，每章新增本次证据支持的校订；17—23 是新增专项章节。原有私有 API、源码和三图结论仍受其 MT8676 版本约束，不能因扩展范围而自动升级为双平台事实。

本次重点更新：Camera Host/Guest 与 DDR 模型；8676/8668 的 Display/Audio/SDK 差异；MBOS 组件退役与端点；三 OS 时间同步和 VSOCK/VSOCKX；Weston/GPU/Mtrace/编解码排查；STR、日志持久化、SELinux/TEE/EINT。最新 PVT 技术分享目录也已重新扫描并单列增量。

证据分为“原始手册中的版本事实”“培训方案/案例”“用户图示”“项目二次归纳”“站点教学方法”“待实机验证”。全文提取不等于逐页审阅，文件新增不等于内容新增；具体来源、哈希、页码、重复与阅读范围见 [22 章](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/22.html)。本次未操作车机，也未把培训脚本视作用户指令。

页码统一按 PDF 物理页序，从封面算第 1 页。源文件保留原位置；文档按来源 ID 链接。旧 SDK 证据 ID 仍可在 [旧版证据索引](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0001.html) 中查找，历史目录只作来源说明。
...
旧版术语扫描覆盖组装后的 00–16 正文与表格（这是旧版流程记录，本次未宣称重跑该 canonical validator），排除 fenced code、inline code、网页标记注释和
...
- 目标平台：MT8676 与 MT8668；原 SDK/UMDP 细节仍限定 MT8676 基线。
...
本版新增 MT8668 与 PVT/MBOS 专项，但每个接口、硬件能力和责任边界仍按对应平台、版本与证据限定。MT8678 或其他平台段落只可在明确标为通用方法且复核适用性后引用，不能写成 MT8676/MT8668 已确认事实。
...
[`evidence/manifest.json`](evidence/旧版证据索引.json) 中解析。一个段落涉及多项材料时，
...
## 2026-09-20 校订：三张基线图之外增加专项视图

本章保留原三图及其节点 ID，作为 MT8676 历史资料基线。本次新增第四张“盟博 OS 服务展开图”，见 [18 章](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/18.html)，并用 MTK Camera 手册和项目 Camera 图建立 [跨域/物理/资源三个视图](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/19.html)。

读图顺序调整为：整机 OS/设备归属 → 原三图模块位置 → MBOS 管理/服务/HAL → Camera 控制与数据通路 → 生命周期与证据。层级图、部署图、业务时序图和容量图回答不同问题，不能用模块位置补出未画明的调用关系。

| 视图 | 能确认 | 本次校订 |
|---|---|---|
| 原三图 | 原始标签、层级、明确连线 | 保留原图/ID，修复图片相对路径 |
| MBOS 新图 | Manager/Service/HAL、Binder 图例、FDBus 端点、退役注释 | DiagnosticManager/Service 不再画为当前必经节点 |
| MT8668 Camera | Android Guest → RpcBinder/VSOCK → Yocto Host | Camera 两域不等于整机只有两 OS |
| Camera DDR | 特定尺寸、格式、fps 和读写次数的预算 | 3+3 与 2+4 PQDIP 分配冲突显式保留 |

依据：[S019 · MT8668_Hypervisor_Camera_User_Manual_CN_V1.0.pdf · PDF第6-8页](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0002.html#page-6) [S038 · MT8668_Hypervisor_System_LLA_User_Manual_CN_V1.1.pdf · PDF第5-9页](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0003.html#page-5) [S296 · MT8668_Camera架构与DDR计算_最终可追溯版.md](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0004.html) [S297 · MT8668最终Camera架构与DDR解读.md](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0005.html)。图中 Qnx 名称不单独证明部署系统是 QNX；硬件引脚、CID、服务名均须记录版本。
...
[原图对照](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/assets/01.jpg)
...
[原图对照](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/assets/02.jpg)
...
[原图对照](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/assets/03.jpg)
...
## 2026-09-20 校订：以资源拥有者核对多 OS 架构

MT8668 System LLA 资料包含 SOS Yocto、UOS Yocto、Android 与 Thyp SDK；Camera 手册只画参与相机的 Yocto Host 和 Android Guest。因此本章扩展到双平台时使用“整机三域、子系统按参与域展开”的描述。[S038 · MT8668_Hypervisor_System_LLA_User_Manual_CN_V1.1.pdf · PDF第5-9页](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0003.html#page-5) [S019 · MT8668_Hypervisor_Camera_User_Manual_CN_V1.0.pdf · PDF第6-8页](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0002.html#page-6)

每个外设建立“物理控制器/IRQ—拥有者—Guest 接口—Host Backend—缓冲共享—恢复 Owner”表。PVT 低速总线资料中虚拟化 SPI 与直通是不同配置：虚拟化由 Guest 前端请求 Host 后端；直通需明确驱动和中断归属，不能让两个系统同时把同一控制器作为独占设备启用。[S283 · 低速总线虚拟化配置.pdf · PDF第4-5页](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0007.html#page-4)

新增 GPU 培训说明其 8676 全虚拟化与半虚拟化方案对 SOS GPU 可用性有不同限制，迁移前须检查 Weston/AVM 等消费者；不能把这一实现选择当作所有 Hypervisor 的定义。[U042 · 全虚拟化和半虚拟化GPU差异.pdf · PDF第4页](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0008.html#page-4) 共享 pCPU 的 period/budget 还需与 Guest 调度一起分析，参见 [S278 · T-Hyper+CPU+调度说明.pdf · PDF第4-6页](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0009.html#page-4) 和 [23 章](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/23.html)。

平台资源差异统一见 [17 章](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/17.html)。以下原有 MT8676 细节继续限定为其资料基线。
...
## 2026-09-20 校订：保留 SDK 基线，新增 8668 迁移入口

本章 Favalon SDK V1.0.166 与 UMDP V1.0.226 的函数、枚举、超时、回调、服务配置仍按 MT8676 基线解释。MT8668 不能凭 API 同名认定兼容，应比较头文件、IDL、二进制 ABI、服务版本、配置和回调线程。

MT8668 General Introduction V1.1 给出的组合为 Android B/16、Yocto 5.0、Kernel 6.12，这是文档参考组合。TBox API 手册强调除 ML_GetModemStat 外，其他 API 正常使用依赖 modem ready，不能将 socket 连接成功当作可调用状态。[S023 · MT8668_Hypervisor_General_Introduction_CN_V1.1.pdf · PDF第4-6页](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0017.html#page-4) [S075 · MT8668_Yocto_T-Box_User_Manual_CN_V1.0.pdf · PDF第5-7页](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0018.html#page-5)

新平台接入顺序建议为：冻结 BSP/SDK/镜像 → 确认服务启动/发现 → 查询 ready → 调用与回调关联 → 验证错误/超时 → 单域重启恢复。原有 SDK 源文件证据仍在 [旧版证据索引](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/source-SRC0001.html)，[17 章](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/17.html) 提供跨平台核对表。
...
[原图对照](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/assets/04.png)
...
[原图对照](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/assets/05.png)
...
[原图对照](https://qiantao18817568425-art.github.io/Architecture_Learning-/architecture/assets/06.png)
