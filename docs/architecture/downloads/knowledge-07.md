...
## T12T 集成补充：仪表 HMI—FDBUS 启动可用性合同

本节把仪表 HMI 在启动早期连接 FDBUS/FDBus 的问题放回 FLOW-01 的
`CanService/FDBus → Cluster/VehicleHAL` 边界中分析。原图支撑 CanService、FDBus
和 Cluster 等模块标签；HMI 的实际进程、FDBUS 的监听方式、registry 名称、端口、启动
顺序和超时值仍须由本车部署清单、IDL/配置与运行日志确认。下列内容是可验证的启动合同，
不是对私有实现的断言。

`FDBUS 进程已启动`、`可建立底层连接`、`已注册到服务目录`、`已能返回当前代快照` 和
`HMI 已完成订阅并消费首个有效样本` 是五个不同状态。现场观察到“启动后首次连接失败，约
2 秒后成功”时，只能说明二者之间存在待解释的可用性窗口；不能据此认定 HMI 或 FDBUS
任一方已经是根因。

```mermaid
flowchart TB
  P["power/reset 与当前 Generation 有效"] --> O["OS/VM 与 IPC 承载可用"]
  O --> S["FDBUS 进程启动"]
  S --> L["listener/transport ready"]
  L --> R["registry/endpoint 注册完成"]
  R --> D["依赖数据与初始快照可用"]
  D --> F["FDBUS business READY"]
  F --> H["HMI 获取 READY，连接/订阅"]
  H --> C["当前代首个有效样本被消费"]
  C --> U["Cluster/HMI Ready"]
```

| 里程碑 | 合格证据 | 不能单独证明 | 首个缺失时的初始责任域 |
|---|---|---|---|
| FDBUS 进程启动 | PID/服务实例、当前 service generation | listener、服务注册或业务可用 | 启动编排/FDBUS |
| listener/transport ready | bind/listen 或双端建链成功 | registry/endpoint 已可发现 | FDBUS/IPC 承载 |
| registry/endpoint ready | 当前服务名、实例、generation 可查询 | 数据源与快照已可用 | FDBUS/服务目录 |
| FDBUS business READY | 依赖状态、服务 health 与首个可读快照 | HMI 已订阅或界面已上屏 | FDBUS 与其依赖服务 |
| HMI 连接/订阅 | 当前 HMI generation 的 connect、subscribe、回调注册 | 回调有本代有效数据 | HMI/SDK Client |
| HMI 消费与显示 | 首样本 sequence/source age、状态机消费、首帧/可见结果 | 上游任一阶段的实现细节 | Cluster/HMI/显示 |

### 启动早连的排查与修复原则

1. 先用 boot ID、VM/service generation、PID/start time 和 suspend/resume 标记区分冷启动、STR 恢复、VM 重建、进程重启；不可把不同启动类型的约 2 秒样本混为同一时延。
2. 在 HMI、FDBUS、其依赖数据服务和电源/显示域记录同一单调时钟，至少记录 HMI 首次 connect、每次重试、FDBUS process start、listener ready、registry ready、business READY、首个 snapshot、subscribe callback 和 HMI 首帧。
3. 若 FDBUS process start 到 business READY 存在窗口，FDBUS 团队需说明窗口内在等待的依赖；若 business READY 已出现而 HMI 没有收到/重试，优先检查 HMI 的 ready 订阅、失败语义和当前 generation；若已经收到首样本但界面未变，回到 FLOW-01 的状态机与显示链。
4. 不以固定 `sleep 2 s` 作为产品合同。应由 FDBUS 发布显式、可查询的当前代 `READY`，HMI 在收到 READY 后连接/订阅；错过事件时使用项目配置的有界退避重试和 health/snapshot 查询。早到请求的 reject、drop、defer 或 queue 语义必须明确，READY 后必须能证明 replay/retrigger/queue flush 的实际结果。
5. READY 超出项目定义的启动预算才进入故障/降级判定。预算内的预就绪连接失败应保留启动指标而非直接报码；预算外的连续失败由本域 Fault Manager 记录事实，再按第 15 章的诊断协调与聚合规则对外呈现。

与 Camera 的边界是：Camera 像素数据面（Sensor/ISP/Buffer）不应因仪表 FDBUS 未 READY
而被隐式阻塞；若 RVC/AVM 的触发、模式或车辆状态经该服务链传递，则控制面必须同样使用
READY、当前 Generation 和首个有效快照。这样既避免早到触发被丢失，也不会把“无画面”
误归因给 Camera 数据面。

[置信度：mixed] 原图 02、03 支撑 FDBus/CanService/Cluster 的可见架构元素；上述 READY、
重放和退避是标准生命周期/IPC 设计约束。当前“约 2 秒”仅为待日志验证的现场现象，不是
平台固定时限。
...
## T12T/MT8668 Camera 架构集成（平台边界隔离）

本手册的主平台是 MT8676；以下内容来自 T12T 已确认的 **MT8668** Camera 架构资料，作为
项目叠加架构单独保存。它不反向证明 MT8676 的 CSI、HAL、DDR、SerDes 或虚拟化实现，两个
平台的共同术语只能作为诊断方法，不能互相替代硬件事实。

T12T 已确认的输入拓扑是四路环视 Camera 经域控侧 MAX96712 聚合：一路 CSI-2 C-PHY/3 trios
给域控本地 SoC；另两路 CSI-2 D-PHY/2 Lane 经两颗 MAX96717，形成 `前+右`、`后+左` 两路
GMSL2 6 Gbps 组合流；MT8668 接收侧 MAX96712 再将两路 `3840×1536` 组合图送至 MT8668
CSI/P1。组合图的方向、ROI、格式、stride/padding、blanking、SOF 和时间戳对齐仍是接口冻结项。

```text
Composite 0: Front + Right  (3840×1536)
Composite 1: Rear  + Left   (3840×1536)
CSI/P1 → Yocto Camera HAL Server/CameraTurboEngine → P2
     → PQDIP: Right/Rear/Left 的 AVM + DVR 输出
     → MML:   Front 的 AVM 输出
Yocto AVM / Android DVR 通过各自消费路径取得结果
```

控制面与数据面必须分开：倒挡/AVM 模式等车辆状态若由 CanService/FDBUS 或等价服务提供，
其启动请求必须等待当前 Generation 的服务 READY 与有效快照；Sensor/SerDes/CSI/ISP/Buffer
数据面仍独立证明首帧、sequence、timestamp、Buffer 所有权和 release。因而“FDBUS 未 READY”
只能解释控制触发未被接收、排队或重放，不能替代 `Camera source-no-frame`、pipeline stop 或
display layer 不可见的证据。

DDR 设计优先级为共享物理输出 buffer，其次由 P2/PQDIP/MML 硬件拆图，避免 App/CPU memcpy；
若声称在完整组合帧落 DDR 前完成流式拆分，必须由 MTK 明确支持。当前 25 fps、
`1920×1536` 单方向图为 73.728 Mpixel/s；PQDIP 单实例三路约 221.184 Mpixel/s。该吞吐是
T12T/MT8668 方案的估算输入，最终以 MTK/PVT 实测和实际格式/stride 为准。

[置信度：T12T 资料确认 + 待实测] 来源为
`Camera架构/MT8668最终Camera架构与DDR解读.md`；已确认拓扑与仍待 MTK/SerDes/应用冻结的
参数在该源文档中分别标注。本节不改变 FLOW-09～17 的 MT8676 证据等级。
...
## T12T 集成补充：电源管理与跨域服务就绪

电源管理负责使电源域、复位、时钟、VM/OS 和显示等前置条件进入允许工作状态；它不能用
“上电完成”替代 FDBUS、HMI、Camera 或诊断服务的业务 READY。对于仪表 HMI—FDBUS
启动问题，必须在同一 Generation 中把下列边界分开：

```text
power/reset valid
→ MCU/SoC/VM generation created
→ OS 与 IPC 承载 ready
→ FDBUS listener/registry/business READY
→ HMI connect/subscribe + initial snapshot
→ HMI first frame/业务 ready
```

| 观察结果 | 可收敛的方向 | 不应直接得出的结论 |
|---|---|---|
| rail/reset、MCU 电源状态或 VM create 缺失 | MCU/Power、BSP、Hypervisor 生命周期 | FDBUS 实现一定有问题 |
| 电源/OS 已 ready，FDBUS 未达到 business READY | FDBUS 启动依赖、服务目录或数据初始化 | HMI 重试策略一定错误 |
| FDBUS READY 已发布，HMI 未建链/订阅 | HMI/SDK Client、ready 事件或 generation 管理 | 电源时序一定异常 |
| 首个快照已到，HMI 无首帧 | 仪表状态机、Window/Weston/显示链 | FDBUS 没有启动 |

每次取证都记录电源状态的**原始值、发布域、状态 request/ACK、rail/reset、display power**，
以及 boot/VM/service/display generation。D1、D2、D3_SA、D3_Sleep、D4 的语义、合法转换
和预算只允许引用当前 T12T 的状态机/ICD；本手册不把其他项目的定义写成事实。服务应通过
显式 READY 和失败原因参与启动编排，不应以固定延时等待电源或依赖服务。Suspend/Resume
后必须建立新会话、重订阅并重新取得初始快照；旧 READY、旧 callback 和旧缓存不得覆盖新代。

Camera 的安全控制面也遵循同一原则：仅当当前电源/显示状态允许且触发状态在当前代有效时，
才启动或恢复 RVC/AVM；Camera 的帧数据面、Buffer 与显示资源另按第 11 章独立证明。这样可
区分“电源状态未允许启动”“控制触发早到未重放”和“Camera 已启动但无帧”三类问题。

[置信度：mixed] FLOW-38 与 `fb_powerMgr` 资料支撑电源/生命周期的既有诊断边界；跨域
READY 合同为标准机制补充，具体 D 状态、服务顺序和预算待 T12T 配置与日志确认。
...
T12T 的诊断资料将“故障事实产生侧、本地 Fault Manager、诊断协调器和 UDS 前端”分开。
该分层补充本章的责任矩阵：诊断协调器负责会话、安全、路由、Request ID、超时、Response
Pending、NRC 映射、域控级聚合与审计；它不替应用、FDBUS 或硬件判断功能是否真正执行。
MCU 负责 MCU/硬件、电源、CAN/LIN、唤醒休眠等本域事实；SOC 负责 OS、进程、资源、
网络和应用服务等本域事实。汇聚节点可缓存、聚合和转发，但不得重复猜测根因或二次报码。

```mermaid
flowchart LR
  T["Tester"] --> F["CAN/DoIP + UDS 前端"] --> C["诊断协调器\n会话/安全/路由/超时/NRC"]
  C --> M["MCU 诊断代理"] --> MA["MCU 应用 / Fault Manager"]
  C --> S["SOC 诊断代理"] --> SA["SOC 应用 / Fault Manager"]
  MA --> C
  SA --> C
```

跨 MCU/SOC 的诊断命令采用 `Request → Ack → Progress → Result`，至少关联 RequestID、
ServiceID/SubFunction、参数、安全等级、会话、Deadline、状态、结果码、失败原因和响应序号。
异步故障事件至少保留 EventID、DTC、状态、BootID、sequence 和冻结帧；断链恢复后要支持
活动故障全量同步与增量重放。大日志/抓包不作为普通内部信号传递，诊断响应只返回任务状态、
文件 ID 或下载句柄。

### 15.9.1 FDBUS 启动可用性案例的诊断规则

对于“仪表 HMI 启动后无法连接 FDBUS，约 2 秒后可连接”的现象，DTC 的事实源不是诊断
协调器，也不是单次 HMI connect 的错误码。先由 HMI/FDBUS 各自记录当前 Generation 的
连接与 READY 里程碑，再按以下规则判定：

| 场景 | 本地事实源与处理 | 对外诊断建议 |
|---|---|---|
| 项目启动预算内，READY 前 connect 被明确拒绝/延后 | HMI/FDBUS 启动指标、原因码、重试次数 | S0 信息事件；不生成 DTC |
| 超出预算仍无 listener/registry/business READY | FDBUS 或其前置依赖的 Fault Manager | 按使能、去抖、确认条件形成 Pending/Active |
| FDBUS READY 已发布，HMI 未连接、未订阅或未重放 | HMI/SDK Client Fault Manager | 记录 HMI 服务消费异常；不可改写 FDBUS 根因 |
| 已有首个快照但状态机/显示未更新 | Cluster/HMI/显示本域 | 按功能降级与用户影响判定 |
| 电源/VM/IPC generation 不连续或重建 | Power/Hypervisor/IPC 本域 | 先定位生命周期事实，其他 timeout 作为传播现象 |

具体启动预算、去抖次数、DTC、冻结帧字段、严重度和 NRC 映射必须来自 T12T 服务目录和诊断
分工表。正常启动瞬态、重连和预期恢复通常是 S0 信息事件；是否升级为 S1/S2 取决于确认条件、
持续时间和功能降级，不能仅依据“约 2 秒”决定。

[置信度：mixed] 本节的职责模型来自 `T12T诊断分工/诊断架构与分工记录.md`；具体 SID/DID/
RID、端口、超时和分工表条目以 `T12T_MCU_SOC_TBOX诊断分工总表` 的目标版本为准。
