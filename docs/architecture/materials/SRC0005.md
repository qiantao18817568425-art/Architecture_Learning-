...
![已确认的 SerDes 物理拓扑](camera-physical-topology-confirmed.jpg)

```mermaid
flowchart LR
    F[环视前<br/>GMSL2 3 Gbps]
    R[环视右<br/>GMSL2 3 Gbps]
    B[环视后<br/>GMSL2 3 Gbps]
    L[环视左<br/>GMSL2 3 Gbps]

    D[MAX96712<br/>域控侧解串/聚合]
    SOC[域控本地 SoC]
    S0[MAX96717<br/>Serializer 0]
    S1[MAX96717<br/>Serializer 1]
    RX[MAX96712<br/>MT8668 接收侧]
    MTK[MT8668<br/>CSI / P1]

    F --> D
    R --> D
    B --> D
    L --> D
    D -->|CSI-2 C-PHY / 3 trios| SOC
    D -->|CSI-2 D-PHY / 2 Lane| S0
    D -->|CSI-2 D-PHY / 2 Lane| S1
    S0 -->|GMSL2 6 Gbps<br/>前+右 3840×1536| RX
    S1 -->|GMSL2 6 Gbps<br/>后+左 3840×1536| RX
    RX -->|CSI-2 两路组合流| MTK
```
...
```mermaid
flowchart TD
    C0[Composite 0<br/>前+右 3840×1536]
    C1[Composite 1<br/>后+左 3840×1536]
    P1[P1<br/>CSI 接收与采集]
    HAL[Yocto Camera HAL Server<br/>CameraTurboEngine]
    P2[P2<br/>后处理编排]
    PQ0[PQDIP HW0]
    PQ1[PQDIP HW1]
    MML[MML]
...
sequenceDiagram
    ...
    participant G as Android Camera Guest
    participant V as RpcBinder / VSOCK
    participant H as Yocto Camera HAL Server
    participant P as P1/P2/PQDIP/MML
    A->>G: configureStreams / Request
    G->>V: 跨域 Request
    V->>H: 合并配置与请求
    H->>P: 建立 pipeline、提交 buffer
    P-->>H: 完成图像处理 / fence signal
    H-->>V: Result + buffer 引用
    V-->>G: 跨域 Result
    G-->>A: 图像帧
```
