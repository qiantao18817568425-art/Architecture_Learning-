...
```mermaid
flowchart TD
    C[环视前/右/后/左<br/>各GMSL2 3Gbps]
    D[MAX96712<br/>域控侧解串与聚合]
    S0[MAX96717<br/>前+右]
    S1[MAX96717<br/>后+左]
    R[MAX96712<br/>MT8668接收侧]
    P1[P1<br/>接收两路3840×1536]
    H[Yocto Camera HAL Server<br/>CameraTurboEngine]
    P2[P2后处理]
    PQ[PQDIP<br/>左/右/后×AVM/DVR=6路]
    M[MML<br/>前视×AVM=1路]
    A[Yocto AVM]
    V[Android DVR]
    ...
    D -->|CSI-2 D-PHY| S0
    D -->|CSI-2 D-PHY| S1
    S0 -->|前+右 3840×1536<br/>GMSL2 6Gbps| R
    S1 -->|后+左 3840×1536<br/>GMSL2 6Gbps| R
    R --> P1 --> H --> P2
    P2 --> PQ
    P2 --> M
    PQ --> A
