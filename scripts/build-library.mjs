...
import {sourceDestination,evidence,buildSources} from './source-library.mjs';
import {diagram,redrawImage,buildDiagramIndex} from './diagram-library.mjs';
...
const search=[];
function render(c){
 const headings=[],counts=new Map();let diagramNumber=0;
 ...
 function dest(href){
  ...
  return sourceDestination(c,href);
 }
 md.use({renderer:{
  ...
  link(token){const url=dest(token.href),label=this.parser.parseInline(token.tokens);return url?`<a href="${esc(url)}">${label}</a>`:`<span class="unresolved-source">${label}<small>（来源条目待核对）</small></span>`;},
  image(token){return redrawImage(token,c);},
  code(token){return token.lang==='mermaid'?diagram(token.text,{id:`${c.edition}-${c.id}-figure-${++diagramNumber}`,title:`${c.title} · 图 ${diagramNumber}`,chapter:(c.edition==='original'?'original-':'')+c.id,source:c.filename}):`<pre><code>${esc(token.text)}</code></pre>`;},
...
function tree(current){return `<details class="nav-tree" open><summary>树状目录 · 全部内容</summary><a class="home-link" href="index.html">学习首页</a>${stages.map(s=>`<details ${s.ids.includes(current)?'open':''}><summary>${s.name}</summary>${s.ids.map(id=>`<a ${id===current?'aria-current="page"':''} href="${id}.html">${id} ${esc(title(id))}</a>`).join('')}</details>`).join('')}<details><summary>原版解释 · 17 章</summary>${data.originals.map(c=>`<a href="${file(c)}">${c.id} ${esc(c.title)}</a>`).join('')}</details><a href="24.html">启动时序逐步解读</a><a href="diagrams.html">全部 Mermaid 图表</a><a href="sources.html">材料原文 · 逐页阅读</a><a href="import.html">ChatGPT 资料导入包</a><a href="../index.html">Day / Week 学习档案 ↗</a></details>`;}
function shell({name,body,current='',toc=''}){return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="真实项目架构学习：完整章节、树状目录、新手导读、MT8676/MT8668 对比和实践方法。"><title>${esc(name)} · 座舱架构实践</title><link rel="stylesheet" href="library.css?v=20260921-layout1"></head><body><a class="skip" href="#main">跳到正文</a><header class="topbar"><a href="index.html" class="brand">座舱架构<span>项目实践知识库</span></a><nav><a href="index.html#learning-path">学习路线</a><a href="17.html">平台对比</a><a href="../index.html">课程档案 ↗</a></nav></header><div class="layout"><aside class="sidebar"><label for="search">全文搜索</label><input id="search" type="search" placeholder="Camera、MBOS、无声…" autocomplete="off"><label class="source-search-toggle"><input id="search-sources" type="checkbox">同时搜索材料原文</label><div id="search-state" role="status"></div><div id="search-results"></div>${tree(current)}</aside><main id="main">${body}</main><aside class="toc"><details open><summary>本章目录</summary>${toc||'<a href="index.html#learning-path">学习路线</a><a href="index.html#method">如何学习真实项目</a><a href="index.html#practice">项目练习</a><a href="index.html#topics">主题索引</a>'}</details></aside></div><footer>资料更新 2026-09-20 · 平台事实、项目证据与教学练习分别记录。阅读标记只保存在本浏览器，不代表已掌握或实机通过。</footer><script src="vendor/mermaid-10.9.5.min.js" defer></script><script src="library.js?v=20260921-layout1" defer></script></body></html>`;}
...
const topicNav=`<section class="guide" id="topics"><h2>按主题学习</h2><p><a href="architecture/index.html"><strong>进入完整架构实践知识库 →</strong></a> · 树状目录、25 章全文、17 章原版对照、新手学习路线。</p><p>架构解释同步：2026-09-20。课程仍到 Day 34，复盘不改变个人掌握记录。</p><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px">${topics.map(([id,name,ids])=>`<section><h3>${id} · ${name}</h3><p>${data.days.filter(d=>d.topics.includes(id)).map(d=>`<a href="#day-${d.day}">Day ${d.day}</a>`).join(' · ')}</p><a href="architecture/${ids[0]}.html">项目架构解释 →</a></section>`).join('')}</div></section>`;
...
writeFileSync(resolve(out,'build-info.json'),JSON.stringify({date:data.date,chapters:data.chapters.length,originals:data.originals.length,searchSections:search.length,courses:50,architecture:'preserved',baseCommit:'a4fe4eaa70ff44aeb9e27068c71514f050f0ba06'},null,2));
const sourceStats=buildSources(shell);const diagramStats=buildDiagramIndex(shell);
const importPage=JSON.parse(readFileSync(resolve(root,'content/import-page.json'),'utf8'));
writeFileSync(resolve(out,'import.html'),shell({name:'ChatGPT 资料导入',body:importPage.body}));
writeFileSync(resolve(out,'material-build-info.json'),JSON.stringify({date:data.date,...sourceStats,...diagramStats},null,2));
console.log(`Built ${all.length} full chapters, learning home, ${search.length} search sections and 50 course anchors.`);
