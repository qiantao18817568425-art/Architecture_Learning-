...
const courseStats = buildCourses();
...
writeFileSync(resolve(out,'build-info.json'),JSON.stringify({date:data.date,chapters:data.chapters.length,originals:data.originals.length,searchSections:search.length,courses:courseStats.count,courseArchiveUpdatedOn:courseStats.updatedOn,architecture:'preserved',baseCommit:'a4fe4eaa70ff44aeb9e27068c71514f050f0ba06'},null,2));
...
console.log(`Built ${all.length} full chapters, learning home, ${search.length} search sections and ${courseStats.count} course entries.`);
