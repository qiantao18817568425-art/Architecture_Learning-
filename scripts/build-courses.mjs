...
export function buildCourses() {
  ...
  const rendered = progress.lessons.map(lesson => {
    ...
    return {lesson, body, block: `<details class="lesson" id="${lesson.id}" data-date="${esc(lesson.date)}"><summary><span class="lesson-title">${esc(lesson.title)}</span><span class="lesson-meta">全文 · 待作答</span></summary><div class="lesson-body"><p class="archive-note">${esc(lesson.date)} · <a href="lessons/${lesson.id}.html">独立阅读本课</a> · 材料发布不代表已经掌握。</p><article>${body}</article></div></details>`};
  });
  ...
  const latest = rendered.at(-1)?.lesson;
  if (latest) archive = archive.replace('<section class="guide">', `<section class="guide"><h2>当前学习任务</h2><p><a href="lessons/${latest.id}.html">${esc(latest.title)}</a> · ${esc(latest.date)} · 下一任务：${esc(progress.nextUnit.title)}</p><p>每日北京时间 20:00 准备课程；作答评审与发布进度分开记录。</p></section>\n<section class="guide">`);
  ...
  for (const {lesson, body} of rendered) {
    writeFileSync(resolve(root, `docs/lessons/${lesson.id}.html`), `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(lesson.title)}</title>${styles}</head><body><main><nav><a href="../index.html#${lesson.id}">课程档案</a> · <a href="../architecture/index.html">项目架构实践</a></nav><article>${body}</article></main><script src="../architecture/vendor/mermaid-10.9.5.min.js"></script><script>mermaid.initialize({startOnLoad:true,theme:'neutral',securityLevel:'strict'});</script></body></html>`);
  }
